// // src/app/programacion-trimestral-poa/page.tsx
// "use client";

// import GenericList from "@/shared/components/common/GenericList";
// import { useProgramacionTrimestral } from "@/modules/poa/hooks/useProgramacionTrimestralMeta";
// import {
//   programacionTrimestralColumns,
//   programacionTrimestralFormFields,
// } from "@/modules/poa/config/programacionTrimestralMeta.config";
// import type { ProgramacionTrimestral } from "@/modules/poa/types/programacionTrimestralMeta.types";

// export default function ProgramacionTrimestralPoaPage() {
//   return (
//     <GenericList<ProgramacionTrimestral>
//       title="Programación Trimestral POA"
//       icon="📅"
//       useData={useProgramacionTrimestral}
//       columns={programacionTrimestralColumns}
//       formFields={programacionTrimestralFormFields}
//       searchKeys={["trimestre", "programado"]}
//       searchPlaceholder="Buscar por trimestre o valor programado..."
//       enableCreate={false}
//       enableEdit
//       enableDelete
//       emptyMessage="No hay programaciones trimestrales registradas."
//       createButtonText="Nueva Programación Trimestral"
//       mapItemToForm={(item: ProgramacionTrimestral) => ({
//         id: item.id,
...
export default function ProgramacionTrimestralPoaPage() {
  ...
  useEffect(() => {
    if (data.length > 0) {
      const grouped = data.reduce((acc: any, prog: ProgramacionTrimestral) => {
        const indicadorId = prog.indicadorPoaId;
        if (!acc[indicadorId]) {
          acc[indicadorId] = {
            ...prog.indicadorPoa,
            programaciones: [],
  ...
  const handleEditStart = (prog: ProgramacionTrimestral) => {
    setEditingProgId(prog.id);
    setEditValue(prog.programado.toString());
  };

  const handleEditSave = async (prog: ProgramacionTrimestral) => {
    setSaving(true);
    try {
  ...
  const handleDelete = async (prog: ProgramacionTrimestral) => {
    if (!confirm(`¿Eliminar la programación del trimestre ${prog.trimestre}?`))
      return;
  ...
  const getTrimestreLabel = (trimestre: number) => {
    ...
    return labels[trimestre] || `T${trimestre}`;
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-slate-800 mb-2">
            Programación de Metas Trimestrales POA
          </h1>
          <p className="text-slate-600">
            Gestión de valores programados por trimestre por indicador POA
          </p>
        </div>
                  ...
                  <div className="flex-1">
                    ...
                    {indicador.actividad && (
                      <div className="mb-3 px-4 py-2 bg-white rounded-lg border border-blue-200">
                        <p className="text-xs text-slate-500 mb-1">Actividad</p>
                        <p className="text-sm font-semibold text-slate-700">
                          {indicador.actividad.description}
                        </p>
                      </div>
                    )}
                  ...
                  <div className="bg-white rounded-lg overflow-hidden border border-slate-200 shadow-sm">
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {indicador.programaciones
                          .sort(
                            (
                              a: ProgramacionTrimestral,
                              b: ProgramacionTrimestral,
                            ) => a.trimestre - b.trimestre,
                          )
                          .map((prog: ProgramacionTrimestral) => (
                            <tr
                              key={prog.id}
                            ...
                            >
                              <td className="px-6 py-4">
                                <span className="font-semibold text-slate-800 text-lg">
                                  {getTrimestreLabel(prog.trimestre)}
                                </span>
                              </td>
                              <td className="px-6 py-4">
                  ...
                  {indicador.programaciones.length === 0 && (
                    <div className="text-center py-12 text-slate-500 bg-white rounded-lg border border-slate-200">
                      ...
                      <p className="font-medium">
                        No hay programaciones trimestrales registradas
                      </p>
                    </div>
                  )}
                </div>
