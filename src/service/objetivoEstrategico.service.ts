...
export const ObjetivoEstrategicoService = {
  ...
  create: async (idOe: number, description: string, politicaDesarrolloId: number) => {
    const mutation = `
    ...
    return fetchGraphQL<any>(mutation, { idOe, description, politicaId: politicaDesarrolloId });
  },
  ...
  filterByPolitica: async (politicaDesarrolloId: number, limit = 100) => {
    ...
    return fetchGraphQL<any>(query, { politicaId: politicaDesarrolloId, limit });
  },
  ...
  utils: {
    ...
    exists: (id: number) => fetchGraphQL<any>(`query Exists($id: ID!) { existsObjetivoEstrategicoAmp(id: $id) }`, { id })
  }
};